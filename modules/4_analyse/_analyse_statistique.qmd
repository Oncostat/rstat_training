...
biomarqueur binaire `bm1`, et un biomarqueur continu `bm2`.
...
#' @param beta_trt,beta_bm1,beta_bm2 the true coefficients
#' @param eos non-informative censoring (end of study)
#' @param seed RNG seed
surv_example = function(n=300, 
                  beta_trt=log(0.7), beta_bm1=log(1.5), beta_bm2=log(0.8),
                  eos=30, seed=42){
  set.seed(seed)
  # Generate covariates
  baseline = tibble(
  ...
  rtn = baseline %>%
    mutate(
      ...
      log_predlin = beta_trt*arm + beta_bm1*bm1 + beta_bm2*bm2,
      lambda0 = log(2)/10 / exp(beta_bm1*mean(bm1) + beta_bm2*mean(bm2)),
      ...
      bm1 = factor(bm1, labels=c("Negative", "Positive")),
    )

  rtn %>%
    select(subjid, arm, t, event, bm1, bm2)
} 
```
...
```{r}
model <- coxph(Surv(t, event) ~ bm2, data=df, x=TRUE)
contsurvplot::plot_surv_area(time="t", status="event", variable="bm2", data=df, model=model)
```
:::
...
fit_adj = coxph(Surv(t, event) ~ arm + bm1 + bm2, data=df)
fit_strat = coxph(Surv(t, event) ~ arm + strata(bm1) + bm2, data=df)
```
...
```{r}
fit_spline = coxph(Surv(t, event) ~ pspline(bm2), data=df)
...
```{r}
fit_num = coxph(Surv(t, event) ~ bm2, data=df)
...
```{r}
fit1  = coxph(Surv(t, event) ~ arm + bm1, data=df)
fit2 = coxph(Surv(t, event) ~ arm + bm1 + bm2, data=df)
anova(fit1, fit2)
```
