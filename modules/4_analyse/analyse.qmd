...
-   Gère quasiment tous les types de variables (continues, discrètes,
    ...)
...
![](media/logo_crosstable.png){fig-align="right" width="120"}
...
```{r}
crosstable(mtcars2, cols=c(mpg, hp), 
           by=cyl) %>% 
...
-   `data` : Jeu de données contenant les variables à analyser, vient
    souvent du pipe.
...
-   `percent_pattern`,`percent_digits` : Contrôle les pourcentages.

-   `funs`, `funs_arg` : Contrôle les calculs sur les variables
    numériques.
...
## Merci de votre attention {.center visibility="uncounted"}

![](media/keep%20calm.jpeg){fig-align="center"}
